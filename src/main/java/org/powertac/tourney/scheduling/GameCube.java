...
public class GameCube
{
  ...
  public GameCube (DbConnection db, int nagents, int[] gametypes, int[] maxarray)
    throws Exception
  {
    noofagents = nagents;
  ...
  private void initializeMap (DbConnection db) throws Exception
  {
    ...
    String sql_load_agents =
      "select distinct AgentType as atype from PowerTAC.AgentQueue order by rand()";
  ...
  /*
   * 
   * returns the game id where (current_games_played) - (max_games_to_play) is
   * max.
   */
  ...
  private AgentLet[] canSchedule (DbConnection db, int[] tmask)
    throws Exception
  {
    AgentLet[] agentlist;
    ...
    String sql_get_query =
      "select InternalAgentID, AgentType from AgentQueue"
              + " where AgentType in " + wherestring + " "
              + " and IsPlaying = 0  " + " group by AgentType ";
    ...
    while (rs.next()) {
      agentlist[i] =
        new AgentLet(rs.getInt("InternalAgentID"), rs.getInt("AgentType"));
      i++;
    }
